...
function makeS3R2(
  ...
  return {
    ...
    async put(key, value, options) {
      ...
      // IMPORTANT:
      // Cloudflare R2 S3 API requires Content-Length
      // for this Vercel/Node upload path.
      //
      // Profile photo upload sends ArrayBuffer,
      // so byteLength gives the exact size.
...
export async function getEnv(): Promise<CfEnv> {
  ...
  return {
    ...
    ADMIN_PASSWORD:
      (ctx.ADMIN_PASSWORD as string) ??
      fromProcess.ADMIN_PASSWORD,

    ADMIN_SESSION_SECRET:
      (ctx.ADMIN_SESSION_SECRET as string) ??
      fromProcess.ADMIN_SESSION_SECRET,
  };
}
...
export async function getDb(): Promise<D1Database | null> {
  ...
  // Vercel / Node -> Cloudflare D1 REST API
  const accountId =
...
export async function getUploads(): Promise<R2Bucket | null> {
  ...
  // Vercel / Node -> Cloudflare R2 S3 API
  const accountId =
    process.env.CLOUDFLARE_ACCOUNT_ID;
